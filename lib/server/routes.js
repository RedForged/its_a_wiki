...
function makeServer(options) {
  ...
  function loadWiki(req, res, next) {
    ...
    if (wiki.visibility === 'private') {
      ...
      const adminOk = user && app.isAdmin(wiki, user);
    ...
    req.wiki = wiki;
    req.user = user;
    req.userIsAdmin = app.isAdmin(wiki, user);
    req.userCanEdit = app.canEdit(wiki, user);
  ...
  router.post('/create', (req, res) => {
    ...
    const theme = String(req.body.theme || 'light').slice(0, 20);
    try {
...
<main class="iaw-page iaw-narrow2">
  ...
  <section class="iaw-card">
    ...
    <table class="iaw-table">
      <thead><tr><th>User</th><th>Joined</th><th>Admin</th><th></th></tr></thead>
      <tbody>${db.allSorted('users', 'created_at', 'desc').slice(0, 50).map((u) => `
        <tr>
  ...
  router.get('/w/:wiki/upload', (req, res) => {
    ...
    res.send(wikiViews.wikiLayout({ wiki: req.wiki, user: req.user, title: 'Upload', body }));
  });
  ...
  router.post('/w/:wiki/admin', (req, res) => {
    ...
    if (req.body.save_settings) {
      ...
      wiki.theme = String(req.body.theme || 'light').slice(0, 20);
  ...
  router.get('/w/:wiki/about', (req, res) => {
    ...
    res.send(wikiViews.wikiLayout({ wiki, user: req.user, title: 'About', body }));
  });
