...
const { regenerateDocsWiki, ensureDocsWiki, DOCS_KEY } = require('../docs-wiki');
...
<main class="iaw-page iaw-narrow2">
  ...
  <section class="iaw-card">
    <h2>All wikis (${db.all('wikis').filter((w) => !w.deleted_at).length})</h2>
    <table class="iaw-table">
      <thead><tr><th>Wiki</th><th>Owner</th><th>Pages</th><th>Edits</th><th>Created</th><th></th></tr></thead>
      <tbody>${db.allSorted('wikis', 'created_at', 'desc').filter((w) => !w.deleted_at).slice(0, 50).map((w) => `
        <tr>
  ...
  <section class="iaw-card">
    ...
    <p class="iaw-muted">The auto-generated documentation lives in the <code>docs</code> wiki. Regenerating rewrites all
    documentation pages from the bundled templates (your manual edits to those pages will be overwritten).</p>
    <form method="post" class="iaw-form iaw-inline-row">
      <button class="iaw-btn" name="regenerate_docs" value="1">Regenerate docs</button>
    </form>
  </section>
  <section class="iaw-card">
  ...
  router.post('/admin', (req, res) => {
    ...
    if (req.body.regenerate_docs) {
      let docsWiki = db.wikiByKey(DOCS_KEY);
      if (docsWiki && docsWiki.deleted_at) {
        // The docs wiki was soft-deleted — restore it so it is visible again.
        delete docsWiki.deleted_at;
        db.put('wikis', docsWiki.id, docsWiki);
      } else if (!docsWiki) {
        // Truly gone — create it fresh.
        ensureDocsWiki(db, app, user);
        docsWiki = db.wikiByKey(DOCS_KEY);
      }
      if (!docsWiki) {
        return instanceAdminPage(req, res, { error: 'Could not create the docs wiki.' });
      }
      const { created, updated } = regenerateDocsWiki(db, app, docsWiki, user);
      return instanceAdminPage(req, res, { ok: `Docs regenerated: ${created} created, ${updated} updated.` });
    }
    if (req.body.delete) {
