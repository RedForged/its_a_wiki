...
const WIKI_SKIN_CSS = `
/* ================================================================
   It's a Wiki! — wiki skin (Default: Dark Theme)
   ================================================================ */
:root {
  --iaw-bg: #131418;
  --iaw-panel: #1d1f25;
  --iaw-sidebar-bg: #17181e;
  --iaw-ink: #e3e5ea;
  --iaw-muted: #959ba5;
  --iaw-line: #2e313a;
  --iaw-accent: #6f9bff;
  --iaw-accent-ink: #0d121c;
  --iaw-heading: #ffffff;
  --iaw-article-width: 1280px;
  --iaw-hero-bg: linear-gradient(120deg, #1c202c, #262936);
  --iaw-hover: rgba(255, 255, 255, 0.07);
  --iaw-th-bg: rgba(255, 255, 255, 0.05);
  --iaw-zebra: rgba(255, 255, 255, 0.02);
  --iaw-code-bg: rgba(255, 255, 255, 0.12);
  --iaw-pre-bg: #252832;
  --iaw-preview-bg: #181b22;
  --iaw-redirect-bg: #2e2612;
  --iaw-redirect-line: #4e401a;
  --iaw-redirect-ink: #fef08a;
  --iaw-blockquote-bg: rgba(255, 255, 255, 0.03);
  --iaw-card-shadow: 0 1px 3px rgba(0, 0, 0, 0.3);
  --iaw-card-shadow-hover: 0 3px 8px rgba(0, 0, 0, 0.45);
}
...
h1, h2, h3, h4, h5, h6 { color: var(--iaw-heading); line-height: 1.25; }
code { font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; background: var(--iaw-code-bg); padding: 1px 5px; border-radius: 4px; font-size: 13px; color: inherit; }

/* ---------------- topbar ---------------- */
.iaw-wikitop {
  background: var(--iaw-panel); border-bottom: 1px solid var(--iaw-line);
  position: sticky; top: 0; z-index: 100; box-shadow: 0 1px 3px rgba(0,0,0,.3);
}
.iaw-wikitop-inner {
...
.iaw-wikilogo-badge {
  width: 30px; height: 30px; border-radius: 8px; background: var(--iaw-accent); color: var(--iaw-accent-ink);
...
.iaw-wikisearch input {
  ...
  font: inherit; width: 200px; background: var(--iaw-bg); color: var(--iaw-ink);
}
.iaw-wikisearch input::placeholder { color: var(--iaw-muted); }
.iaw-wikisearch input:focus { outline: none; border-color: var(--iaw-accent); }
.iaw-userchip { display: inline-flex; align-items: center; gap: 7px; font-weight: 600; font-size: 13.5px; color: var(--iaw-ink); }
.iaw-userchip a { color: var(--iaw-ink); }
.iaw-userchip a:hover { color: var(--iaw-accent); text-decoration: none; }
...
.iaw-btn-primary { background: var(--iaw-accent); border-color: var(--iaw-accent); color: var(--iaw-accent-ink); font-weight: 600; }
...
.iaw-sidebar a:hover { background: var(--iaw-hover); text-decoration: none; }
...
.iaw-tab:hover { background: var(--iaw-hover); color: var(--iaw-ink); text-decoration: none; }
...
.iaw-tab.iaw-watching { color: #facc15; }
.iaw-tab.iaw-tab-danger { color: #f87171; }
...
.iaw-article {
  ...
  padding: 26px 38px 30px; margin-top: 4px; box-shadow: var(--iaw-card-shadow);
}
...
.iaw-article-body blockquote { border-left: 3px solid var(--iaw-accent); margin: 1em 0; padding: 6px 16px; color: var(--iaw-ink); background: var(--iaw-blockquote-bg); border-radius: 0 6px 6px 0; }
...
.iaw-toc .iaw-toc-title { font-weight: 700; font-size: 13px; margin-bottom: 2px; color: var(--iaw-heading); }
...
/* tables */
.iaw-article-body table.iaw-table { border-collapse: collapse; margin: 1em 0; width: 100%; }
.iaw-article-body th, .iaw-article-body td { border: 1px solid var(--iaw-line); padding: 6px 12px; }
.iaw-article-body th { background: var(--iaw-th-bg); text-align: left; color: var(--iaw-heading); }
.iaw-article-body tr:nth-child(even) td { background: var(--iaw-zebra); }
...
.iaw-article-body pre {
  background: var(--iaw-pre-bg); border: 1px solid var(--iaw-line); border-radius: 8px;
  padding: 10px 14px; overflow-x: auto; font: 13px/1.5 ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
  color: var(--iaw-ink);
}
.iaw-article-body pre code { background: transparent; padding: 0; }
...
.iaw-catlabel { font-weight: 700; color: var(--iaw-heading); }
...
.iaw-catlink:hover { border-color: var(--iaw-accent); background: var(--iaw-hover); text-decoration: none; }

/* redirect / missing page */
.iaw-redirect { background: var(--iaw-redirect-bg); border: 1px solid var(--iaw-redirect-line); color: var(--iaw-redirect-ink); border-radius: 8px; padding: 8px 12px; margin-bottom: 14px; font-size: 14px; }
.iaw-redirect a { color: var(--iaw-accent); font-weight: 600; }
.iaw-missingpage { padding: 18px 0 8px; }

/* ---------------- flash toast (one-shot, fixed, auto-dismiss) ---------------- */
.iaw-flash {
  position: fixed; top: 64px; right: 18px; z-index: 300;
  background: #1e293b; color: #f8fafc; border: 1px solid #334155; padding: 10px 18px; border-radius: 10px;
  box-shadow: 0 8px 28px rgba(0,0,0,.45); font-size: 14px;
...
.iaw-error { background: #371717; border: 1px solid #5c2424; color: #fca5a5; padding: 10px 14px; border-radius: 8px; margin: 12px 0; }
.iaw-warn { background: #34280f; border: 1px solid #574218; color: #fde047; padding: 10px 14px; border-radius: 8px; }
...
.iaw-searchbar input { flex: 1; min-width: 200px; padding: 8px 12px; border: 1px solid var(--iaw-line); border-radius: 8px; font: inherit; background: var(--iaw-panel); color: var(--iaw-ink); }
.iaw-searchbar select { padding: 8px; border: 1px solid var(--iaw-line); border-radius: 8px; background: var(--iaw-panel); color: var(--iaw-ink); font: inherit; }
.iaw-searchbar input:focus, .iaw-searchbar select:focus { outline: none; border-color: var(--iaw-accent); }

.iaw-pagelist { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 12px; }
.iaw-pagecard { background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 8px; padding: 12px 16px; display: flex; flex-direction: column; gap: 3px; color: inherit; box-shadow: var(--iaw-card-shadow); transition: box-shadow .12s, border-color .12s; }
.iaw-pagecard:hover { border-color: var(--iaw-accent); text-decoration: none; box-shadow: var(--iaw-card-shadow-hover); }
.iaw-pagecard b { font-size: 15px; color: var(--iaw-heading); }

.iaw-recent { display: flex; flex-direction: column; gap: 8px; }
.iaw-recentitem { display: flex; gap: 10px; align-items: flex-start; background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 8px; padding: 10px 14px; font-size: 14px; box-shadow: var(--iaw-card-shadow); }
.iaw-recentmeta { color: var(--iaw-muted); font-size: 12.5px; }

.iaw-table { border-collapse: collapse; width: 100%; background: var(--iaw-panel); border: 1px solid var(--iaw-line); font-size: 14px; color: var(--iaw-ink); }
.iaw-table th, .iaw-table td { border: 1px solid var(--iaw-line); padding: 7px 10px; text-align: left; }
.iaw-table th { background: var(--iaw-th-bg); color: var(--iaw-heading); }
.iaw-history td { font-size: 13.5px; }

.iaw-letterbar { display: flex; gap: 4px; flex-wrap: wrap; margin: 12px 0; }
.iaw-letterbar a { padding: 4px 9px; border: 1px solid var(--iaw-line); border-radius: 6px; background: var(--iaw-panel); color: var(--iaw-ink); }
.iaw-letterbar a:hover { border-color: var(--iaw-accent); text-decoration: none; }
.iaw-letterbar a.iaw-active { background: var(--iaw-accent); border-color: var(--iaw-accent); color: var(--iaw-accent-ink); }
...
.iaw-diffcol { background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 8px; padding: 14px 20px; box-shadow: var(--iaw-card-shadow); }
...
.iaw-mediacard { background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 10px; overflow: hidden; box-shadow: var(--iaw-card-shadow); }
.iaw-media-thumb { display: flex; height: 140px; background: var(--iaw-bg); align-items: center; justify-content: center; }
.iaw-media-thumb img { max-width: 100%; max-height: 140px; object-fit: cover; }
.iaw-fileicon { font-weight: 800; color: var(--iaw-muted); font-size: 18px; }
...
/* forms / edit page */
.iaw-form label { display: block; margin: 10px 0; font-weight: 600; color: var(--iaw-heading); }
...
.iaw-form textarea, .iaw-form select {
  ...
  border-radius: 8px; font: inherit; background: var(--iaw-panel); color: var(--iaw-ink); font-weight: 400;
}
.iaw-form input::placeholder, .iaw-form textarea::placeholder { color: var(--iaw-muted); }
.iaw-form input:focus, .iaw-form textarea:focus, .iaw-form select:focus { outline: none; border-color: var(--iaw-accent); }
...
.iaw-card { background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 10px; padding: 20px 24px; margin: 16px 0; box-shadow: var(--iaw-card-shadow); }
...
.iaw-banned { color: #f87171; font-weight: 600; }
.iaw-danger-zone { border-color: #5c2424; }
...
.iaw-preview { border: 2px dashed var(--iaw-accent); border-radius: 10px; padding: 16px 20px; margin: 14px 0; background: var(--iaw-preview-bg); }
.iaw-preview h2 { color: var(--iaw-heading); margin-top: 0; }
.iaw-edittips { background: var(--iaw-panel); border: 1px solid var(--iaw-line); border-radius: 10px; padding: 14px 18px; margin-top: 18px; font-size: 13.5px; box-shadow: var(--iaw-card-shadow); }
.iaw-edittips code { background: var(--iaw-code-bg); padding: 1px 5px; border-radius: 4px; }

/* hub (wiki front page) */
.iaw-hub-hero { background: var(--iaw-hero-bg); border-bottom: 1px solid var(--iaw-line); border-radius: 0 0 10px 10px; }
...
.iaw-hub-hero h1 { margin: 0; font-size: 28px; color: var(--iaw-heading); }
...
.iaw-section-head h2 { font-size: 19px; margin: 0; color: var(--iaw-heading); }

/* wiki footer */
.iaw-wikifooter { border-top: 1px solid var(--iaw-line); background: var(--iaw-panel); margin-top: 30px; flex-shrink: 0; }
.iaw-wikifooter-inner { width: 100%; padding: 14px 22px; display: flex; justify-content: space-between; gap: 10px; font-size: 13px; color: var(--iaw-muted); flex-wrap: wrap; box-sizing: border-box; }
.iaw-wikifooter a { color: var(--iaw-muted); text-decoration: underline; }
.iaw-wikifooter a:hover { color: var(--iaw-accent); }

.iaw-template-usage { border: 1px dashed var(--iaw-line); padding: 8px 12px; border-radius: 8px; background: var(--iaw-bg); }
...
body.theme-forest {
  --iaw-bg: #141c14;
  --iaw-panel: #1b261b;
  --iaw-sidebar-bg: #162016;
  --iaw-ink: #e3ece3;
  --iaw-muted: #8ea88d;
  --iaw-line: #2d3e2c;
  --iaw-accent: #4ade80;
  --iaw-accent-ink: #052e16;
  --iaw-heading: #f0fdf4;
  --iaw-hero-bg: linear-gradient(120deg, #172917, #1f361f);
  --iaw-hover: rgba(74, 222, 128, 0.1);
  --iaw-th-bg: rgba(74, 222, 128, 0.07);
  --iaw-zebra: rgba(74, 222, 128, 0.03);
  --iaw-code-bg: rgba(74, 222, 128, 0.12);
  --iaw-pre-bg: #172417;
  --iaw-preview-bg: #182718;
  --iaw-redirect-bg: #2b2b10;
  --iaw-redirect-line: #4c4d18;
  --iaw-redirect-ink: #fef08a;
  --iaw-blockquote-bg: rgba(74, 222, 128, 0.04);
  --iaw-card-shadow: 0 1px 3px rgba(0, 0, 0, 0.35);
  --iaw-card-shadow-hover: 0 3px 8px rgba(0, 0, 0, 0.5);
}
`;
