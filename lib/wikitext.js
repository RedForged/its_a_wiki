...
function makeRenderer(options) {
  ...
  function inline(text, ctx, depth, state) {
    ...
    while (i < t.length) {
      ...
      // external links [url text], bare https://…, www., mailto:
      if (t[i] === '[' && c2 !== '[[') {
      ...
      // bare URL: http(s)://, ftp://, mailto:, or www.
      const bareUrl = /^(?:https?:\/\/|ftp:\/\/|mailto:)[^\s<>"')]+|^www\.[^\s<>"')]+/i.exec(t.slice(i));
      if (bareUrl) {
        let raw = bareUrl[0];
        // strip trailing punctuation that is not part of the URL
        let m = /[.,;:!?]+$/.exec(raw);
        if (m) raw = raw.slice(0, -m[0].length);
        // degenerate after stripping (e.g. "https://," → "https://") — leave as text
        if (raw.length < 8 || raw.endsWith('://') || raw === 'mailto:' || raw === 'www.') {
          out += escapeHtml(bareUrl[0]);
          i += bareUrl[0].length;
          continue;
        }
        const href = raw;
        out += `<a class="iaw-extlink" href="${escapeHtml(href)}" target="_blank" rel="noopener nofollow">${escapeHtml(raw)}</a>`;
        i += raw.length;
        continue;
      }
      if (t[i] === '&') {
  ...
  function isSafeUrl(url) {
    return /^(https?|ftp):\/\/|^mailto:/i.test(url) && !/[<>"']/.test(url);
  }
