...
function makeRenderer(options) {
  ...
  function inline(text, ctx, depth, state) {
    ...
    // protect any HTML produced by template expansion from the text scanner
    const placeholders = [];
    ...
    while (i < t.length) {
      ...
      if (c2 === "''") {
        // bold / italic runs — match longest first
        const m = /'''(.*?)'''|''(.*?)''/s.exec(t.slice(i));
        if (m) {
          const tag = m[1] !== undefined ? 'b' : 'i';
          const content = m[1] !== undefined ? m[1] : m[2];
          out += `<${tag}>${inline(content, ctx, depth, state)}</${tag}>`;
          i += m[0].length;
          continue;
        }
        out += "''";
    ...
    // underline / strike plain-text fallbacks for <u>/<s>
    return out;
  }
  ...
  function inlineLink(inner, ctx, depth, state) {
    ...
    const label = rest.length ? rest[0] : cls.title;
    const href = `/w/${encodeURIComponent(ctx.wikiKey || '')}/p/${encodeURIComponent(cls.title)}`;
    const redirCls = cls.isRedir ? ' iaw-redir' : '';
    let title = escapeHtml(label);
    if (rest.length && rest[0].trim() === '' && target) title = escapeHtml(cls.title);
    if (isSelf) {
      return `<b>${escapeHtml(label || cls.title)}</b>`;
    }
    return `<a class="iaw-link${redirCls}" href="${href}">${inline(title, ctx, depth, state)}</a>`;
  }
