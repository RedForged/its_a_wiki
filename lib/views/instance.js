  ...
  <form method="post" action="/create" class="iaw-form iaw-card">
    ...
    <label>Theme
      <select name="theme">
        ...
        <option value="dark" ${f.theme === 'dark' ? 'selected' : ''}>Dark</option>
        <option value="ocean" ${f.theme === 'ocean' ? 'selected' : ''}>Ocean</option>
        <option value="forest" ${f.theme === 'forest' ? 'selected' : ''}>Forest</option>
        <option value="sunset" ${f.theme === 'sunset' ? 'selected' : ''}>Sunset</option>
      </select>
    </label>
...
function aboutPage({ user }) {
  const body = `<main class="iaw-page iaw-narrow">
    ...
    <p><b>It's a Wiki!</b> is an open-source (MIT) wiki farm — a modern, self-hostable successor
