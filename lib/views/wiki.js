...
/* You can style anything that has an iaw- class, e.g. tables, images, links… */">${h(css)}</textarea>
    ...
    <script>
    (function () {
      // Auto-linkify bare URLs pasted into the wikitext editor:
      // "https://example.com" → "[https://example.com https://example.com]"
      // (only text that is not already inside a [..] or [[..]] link).
      const ta = document.querySelector('.iaw-wikitext-editor');
      if (!ta) return;
      const URL_RE = /(^|[^[\]a-zA-Z0-9_])((?:https?:\/\/|ftp:\/\/|mailto:)[^\s<>"'\]\[]+|www\.[^\s<>"'\]\[]+)(?=$|[\s,.;:!?)])/g;
      ta.addEventListener('paste', (e) => {
        ...
        const pasted = clip.replace(URL_RE, (m, pre, url) => {
          // trim trailing punctuation from the URL itself
          let u = url.replace(/[.,;:!?]+$/, '');
          if (u.length < 8 || u.endsWith('://') || u === 'mailto:' || u === 'www.') return m;
          return pre + '[' + u + ' ' + u + ']';
        });
        ta.value = before + pasted + after;
  ...
  <div class="iaw-edittips">
    ...
    <ul>
      <li><code>'''bold'''</code> · <code>''italic''</code> · <code>'''''both'''''</code> · <code>++underline++</code> · <code>--strike--</code></li>
      <li><code>[[Page name]]</code> link · <code>[[Page name|custom label]]</code> · <code>[[Page name|]]</code> pipe trick</li>
      <li><code>[https://example.com link text]</code> external link</li>
      <li><code>&lt;u&gt;&lt;s&gt;&lt;sub&gt;&lt;sup&gt;&lt;code&gt;</code> inline HTML</li>
      <li><code>== Heading ==</code> (upto 6 =)</li>
      <li><code>[[File:img.jpg|thumb|caption]]</code> image</li>
      <li><code>{{Template|arg=value}}</code> template · <code>{{PAGENAME}}</code> magic word</li>
      ...
      <li><code>&lt;nowiki&gt;</code> raw · <code>&lt;ref&gt;…&lt;/ref&gt;</code> · <code>&lt;gallery&gt;</code></li>
    </ul>
  </div>
</div>`;
