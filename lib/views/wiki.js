...
<div class="iaw-wiki-shell">
  <aside class="iaw-sidebar">
    <div class="iaw-sidebar-section">
      ...
      <a href="/w/${h(wiki.key)}/about"><span class="iaw-sidebar-icon">ⓘ</span> About</a>
    </div>
    <div class="iaw-sidebar-section">
    ...
    ${admin ? `<div class="iaw-sidebar-section">
      ...
      <a href="/w/${h(wiki.key)}/upload"><span class="iaw-sidebar-icon">↑</span> Upload file</a>
      <a href="/w/${h(wiki.key)}/admin"><span class="iaw-sidebar-icon">⚙</span> Admin console</a>
    </div>` : ''}
  </aside>
...
<body class="iaw-wiki theme-${h(wiki.theme || 'light')} ${banner || ''}" data-wiki-key="${h(wiki.key)}">
...
<div class="iaw-page iaw-narrow2">
  ...
  return wikiLayout({ wiki, user, title: 'Category: ' + category, body, active: 'all' });
}
...
<div class="iaw-page iaw-narrow2">
  ...
  return wikiLayout({ wiki, user, title: 'Recent changes', body, active: 'all' });
}
...
<div class="iaw-page">
  ...
  return wikiLayout({ wiki, user, title: 'Media', body, active: 'all' });
}
...
<div class="iaw-page iaw-narrow2">
  ...
  return wikiLayout({ wiki, user, title: 'File: ' + media.name, body, active: 'all' });
}

function specialPage({ wiki, user, name, body }) {
  return wikiLayout({ wiki, user, title: 'Special: ' + name, body, active: 'all' });
}
...
<div class="iaw-page iaw-narrow2">
  ...
  <section class="iaw-card">
    ...
    <form method="post" action="/w/${h(wiki.key)}/admin" class="iaw-form">
      ...
      <label>Theme<select name="theme">
        ${['light', 'dark', 'ocean', 'forest', 'sunset'].map((t) => `<option ${wiki.theme === t ? 'selected' : ''}>${t}</option>`).join('')}
      </select></label>
  ...
  return wikiLayout({ wiki, user, title: 'Admin', body, active: '' });
}
...
function membersPage({ wiki, user, users }) {
  ...
  return wikiLayout({ wiki, user, title: 'Members', body });
}
